...
class ContextualElements:
    ...
    def to_dict(self) -> dict[str, Any]:
        """转换为可JSON序列化的字典"""
        return {
            "time_dimension": self.time_dimension.value
...
class ContextProfile:
    ...
    def to_dict(self) -> dict[str, Any]:
        """转换为可JSON序列化的字典"""
        return {
            "task_characteristics": self.task_characteristics.to_dict()
            if self.task_characteristics
            else None,
            "contextual_elements": self.contextual_elements.to_dict()
            if self.contextual_elements
            else None,
            "cognitive_needs": self.cognitive_needs.to_dict()
            if self.cognitive_needs
            else None,
...
class ContextAnalysisModule(ABC):
    ...
    @abstractmethod
    def extract_contextual_elements(
        self, query: str, context: dict[str, Any]
    ) -> ContextualElements:
        """提取情境要素"""
        pass
...
class TaskTypeIdentifier(ContextAnalysisModule):
    ...
    def _classify_task_type(self, query: str, user_profile=None) -> TaskType:
        ...
        # 基于查询关键词进行基础分类
        if any(
            keyword in query for keyword in ["如何看待", "可能有哪些", "有什么", "探索"]
        ):
    ...
    def _assess_openness(self, query: str, user_profile=None) -> float:
        ...
        if user_profile and user_profile.cognitive_characteristics:
            if (
                user_profile.cognitive_characteristics.get("cognitive_complexity", 0.5)
                > 0.7
            ):
    ...
    def _assess_creativity_need(self, query: str, user_profile=None) -> float:
        ...
        if user_profile and user_profile.cognitive_characteristics:
            creativity_need += (
                user_profile.cognitive_characteristics.get("creativity_tendency", 0.5)
                * 0.3
            )
    ...
    def predict_cognitive_needs(
        ...
        return CognitiveNeeds(
            memory_activation_needs={"surface": 0.3, "deep": 0.4, "meta": 0.3},
...
class ContextualElementExtractor(ContextAnalysisModule):
    ...
    def _analyze_abstraction_level(
        self, query: str, user_profile=None
    ) -> AbstractionLevel:
        """分析抽象层次"""
        # 基于查询关键词判断抽象层次
        if any(keyword in query for keyword in ["具体", "步骤", "操作", "实现"]):
            ...
            if user_profile and user_profile.cognitive_characteristics:
                if (
                    user_profile.cognitive_characteristics.get("abstraction_level", 0.5)
                    > 0.7
                ):
                    return AbstractionLevel.META
...
class CognitiveNeedsPredictor(ContextAnalysisModule):
    ...
    def predict_cognitive_needs(
        ...
        """
        knowledge_needs = self._predict_knowledge_supplement(
            task_characteristics, contextual_elements
        )
        framework_needs = self._predict_thinking_framework(
            task_characteristics, contextual_elements
        )
        creativity_needs = self._predict_creativity_stimulation(
            task_characteristics, contextual_elements
        )
    ...
    def _predict_knowledge_supplement(
        self,
        task_characteristics: TaskCharacteristics,
        contextual_elements: ContextualElements,
    ) -> list[str]:
    ...
    def _predict_thinking_framework(
        self,
        task_characteristics: TaskCharacteristics,
        contextual_elements: ContextualElements,
    ) -> list[str]:
    ...
    def _predict_creativity_stimulation(
        self,
        task_characteristics: TaskCharacteristics,
        contextual_elements: ContextualElements,
    ) -> list[str]:
...
class IntegratedContextAnalyzer:
    ...
    def analyze_context(self, query: str, context: dict[str, Any]) -> ContextProfile:
        ...
        # 提取情境要素
        contextual_elements = self.element_extractor.extract_contextual_elements(
            query, context
        )

        # 预测认知需求
        cognitive_needs = self.needs_predictor.predict_cognitive_needs(
            task_characteristics, contextual_elements
        )
