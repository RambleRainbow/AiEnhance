...
from .interfaces import (
    CognitiveChallenger, ChallengeRequest, ChallengeResult,
    Challenge, ChallengeType, CollaborationContext
)
...
class CognitiveChallenge(CognitiveChallenger):
        ...
        try:
            ...
            for challenge_type in challenge_types:
                try:
                    ...
                    logger.warning(f"Failed to generate {challenge_type.value} challenge: {e}")
                    continue
            
            # 生成元认知反思
            meta_reflection = await self._generate_meta_reflection(challenges, reasoning_analysis)
            
            # 识别成长机会
            growth_opportunities = await self._identify_growth_opportunities(challenges)
            
            # 提供下一步建议
            next_steps = await self._generate_next_steps(challenges, request.intensity_level)
            ...
            return ChallengeResult(
                ...
                challenge_metadata={
                    ...
                    "challenge_types_generated": [c.challenge_type.value for c in challenges]
                }
            )
    ...
    async def _analyze_reasoning(self, content: str, user_reasoning: Optional[str]) -> Dict[str, Any]:
        """分析用户的推理过程"""
        prompt = f"""
请分析以下内容中的推理过程：
...
"""
    ...
    def _parse_reasoning_analysis(self, response: str) -> Dict[str, Any]:
        ...
        for line in lines:
            ...
            elif any(keyword in line.lower() for keyword in ['推理', 'reasoning', '逻辑']):
    ...
    async def _generate_challenge_by_type(self, request: ChallengeRequest,
                                        reasoning_analysis: Dict[str, Any],
                                        challenge_type: ChallengeType) -> Optional[Challenge]:
        """根据类型生成特定挑战"""
        
        if challenge_type == ChallengeType.ASSUMPTION_QUESTIONING:
            return await self._generate_assumption_challenge(request, reasoning_analysis)
        elif challenge_type == ChallengeType.BLIND_SPOT_DETECTION:
            return await self._generate_blind_spot_challenge(request, reasoning_analysis)
        elif challenge_type == ChallengeType.COMPLEXITY_EXPANSION:
            return await self._generate_complexity_challenge(request, reasoning_analysis)
    ...
    async def _generate_assumption_challenge(self, request: ChallengeRequest,
                                           reasoning_analysis: Dict[str, Any]) -> Optional[Challenge]:
        """生成假设质疑挑战"""
...
"""
        ...
        return Challenge(
            ...
            expansion_directions=["重新审视基础假设", "探索对立假设", "构建新的前提框架"]
        )
    
    async def _generate_blind_spot_challenge(self, request: ChallengeRequest,
                                           reasoning_analysis: Dict[str, Any]) -> Optional[Challenge]:
        """生成盲点检测挑战"""
        prompt = f"""
识别以下内容中可能存在的思维盲点：
...
"""
        ...
        return Challenge(
            ...
            expansion_directions=["扩展时间视野", "多角度思考", "跨文化理解", "情感因素考虑"]
        )
    
    async def _generate_complexity_challenge(self, request: ChallengeRequest,
                                           reasoning_analysis: Dict[str, Any]) -> Optional[Challenge]:
        """生成复杂性扩展挑战"""
        prompt = f"""
将以下问题的复杂性进行扩展：
...
"""
        ...
        return Challenge(
            ...
            expansion_directions=["系统性思考", "动态分析", "多目标平衡", "不确定性管理"]
        )
    
    async def _generate_creative_challenge(self, request: ChallengeRequest,
                                         reasoning_analysis: Dict[str, Any]) -> Optional[Challenge]:
        """生成创意激发挑战"""
        prompt = f"""
为以下内容提供创意激发挑战：
...
"""
    ...
    def _extract_questions(self, response: str) -> List[str]:
        ...
        if not questions:
            for line in lines:
                line = line.strip()
                if any(word in line for word in ['如何', '为什么', '什么', '是否', '会不会', 'how', 'why', 'what', 'whether']):
                    clean_line = re.sub(r'^(\d+\.|[-*•])\s*', '', line)
                    if clean_line:
                        questions.append(clean_line)
    ...
    def _extract_frameworks(self, response: str) -> List[str]:
        ...
        for line in lines:
            line = line.strip()
            if any(keyword in line.lower() for keyword in ['框架', 'framework', '模型', 'model', '方法', 'approach']):
                clean_line = re.sub(r'^(\d+\.|[-*•])\s*', '', line)
    ...
    async def _generate_meta_reflection(self, challenges: List[Challenge],
                                      reasoning_analysis: Dict[str, Any]) -> str:
        """生成元认知反思"""
        if not challenges:
...
"""
    ...
    async def _identify_growth_opportunities(self, challenges: List[Challenge]) -> List[str]:
        """识别成长机会"""
        opportunities = []
    ...
    async def _generate_next_steps(self, challenges: List[Challenge], intensity_level: str) -> List[str]:
        """生成下一步建议"""
        next_steps = []
