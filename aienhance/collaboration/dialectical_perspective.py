...
class DialecticalPerspectiveGenerator(PerspectiveGenerator):
    ...
    async def generate_perspectives(
        ...
        try:
            ...
            content_analysis = await self._analyze_content(
                request.content, request.user_position
            )

            # 2. 生成不同类型的视角
            for perspective_type in request.perspective_types or [
                PerspectiveType.OPPOSING,
                PerspectiveType.MULTI_DISCIPLINARY,
            ]:
                if perspective_type == PerspectiveType.OPPOSING:
                    opposing_perspectives = await self._generate_opposing_views(
                    ...
                    disciplinary_perspectives = await self._generate_disciplinary_views(
                        request, content_analysis
                    )
                    perspectives.extend(disciplinary_perspectives)

                elif perspective_type == PerspectiveType.STAKEHOLDER:
                    stakeholder_perspectives = await self._generate_stakeholder_views(
                        request, content_analysis
                    )
            ...
            if len(perspectives) > request.max_perspectives:
                perspectives = sorted(
                    perspectives, key=lambda p: p.relevance_score, reverse=True
                )[: request.max_perspectives]
            ...
            return PerspectiveResult(
                ...
                generation_metadata={
                    ...
                    "generation_strategies_used": [
                        p.perspective_type.value for p in perspectives
                    ],
    ...
    async def _analyze_content(
        self, content: str, user_position: str | None = None
    ) -> dict[str, Any]:
        """分析内容和用户立场"""
        analysis_prompt = f"""
...
"""
    ...
    async def _generate_opposing_views(
        self, request: PerspectiveRequest, content_analysis: dict[str, Any]
    ) -> list[Perspective]:
        """生成对立观点"""
        ...
        for strategy in self.opposition_strategies[:2]:  # 限制策略数量
            try:
                ...
                logger.warning(
                    f"Failed to generate opposing view with strategy {strategy}: {e}"
                )
                continue

        return opposing_perspectives

    async def _generate_opposing_view_by_strategy(
        self, content: str, content_analysis: dict[str, Any], strategy: str
    ) -> Perspective | None:
        """使用特定策略生成对立观点"""
        strategy_prompts = {
            "direct_negation": f"""
...
""",
        ...
        return self._parse_perspective_response(
            response.content, PerspectiveType.OPPOSING, strategy
        )

    async def _generate_disciplinary_views(
        self, request: PerspectiveRequest, content_analysis: dict[str, Any]
    ) -> list[Perspective]:
        """生成多学科视角"""
        ...
        for discipline in disciplines[:2]:  # 限制学科数量
            try:
                ...
                logger.warning(
                    f"Failed to generate {discipline.value} perspective: {e}"
                )
                continue

        return disciplinary_perspectives

    async def _generate_disciplinary_view(
        self,
        content: str,
...
"""
        ...
        perspective = self._parse_perspective_response(
            response.content, PerspectiveType.MULTI_DISCIPLINARY, discipline.value
        )
    ...
    async def _generate_stakeholder_views(
        self, request: PerspectiveRequest, content_analysis: dict[str, Any]
    ) -> list[Perspective]:
        """生成利益相关者视角"""
        # 识别潜在利益相关者
        ...
        for stakeholder in stakeholders[:2]:  # 限制数量
            try:
                ...
                logger.warning(
                    f"Failed to generate stakeholder view for {stakeholder}: {e}"
                )
                continue
...
"""
    ...
    async def _generate_stakeholder_view(
        self, content: str, stakeholder: str
    ) -> Perspective | None:
        """生成特定利益相关者视角"""
        prompt = f"""
...
"""
        ...
        perspective = self._parse_perspective_response(
            response.content, PerspectiveType.STAKEHOLDER, stakeholder
        )
    ...
    def _parse_perspective_response(
        self, response: str, perspective_type: PerspectiveType, identifier: str
    ) -> Perspective | None:
        """解析视角生成响应"""
        try:
            # 简单的响应解析
            ...
            for line in lines:
                if any(
                    ...
                    for keyword in ["论据", "argument", "观点", "view"]
                ):
                    current_section = "arguments"
                elif any(
                    keyword in line.lower()
                    for keyword in ["证据", "evidence", "例子", "example"]
                ):
...
"""
        ...
        for i, p1 in enumerate(perspectives):
            for p2 in perspectives[i + 1 :]:
                if p1.perspective_type != p2.perspective_type:
                    tension = (
                        f"{p1.title} vs {p2.title}: 在核心假设和价值取向上存在根本分歧"
                    )
    ...
    async def _generate_integration_suggestions(
        ...
        # 基于具体视角类型添加建议
        has_opposing = any(
            p.perspective_type == PerspectiveType.OPPOSING for p in perspectives
        )
        has_disciplinary = any(
            p.perspective_type == PerspectiveType.MULTI_DISCIPLINARY
